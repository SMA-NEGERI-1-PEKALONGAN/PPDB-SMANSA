...
<body class="sidebar-shrink">
    ...
    function addDataUniqueIdToLocalStorage() {
        ...
        // Fungsi helper untuk generate ID baru, simpan ke local storage, dan kirim AJAX
        const generateAndSendUID = () => {
            // Menambahkan Date.now() agar ID jauh lebih unik dan mencegah bentrok
            const newUniqueId = 'UID-' + Math.random().toString(36).substr(2, 9) + '-' + Date.now().toString(36);

            localStorage.setItem('unique_id', newUniqueId);
            // Simpan format string hari ini (contoh: "Thu Mar 12 2026")
            localStorage.setItem('unique_id_date', currentDate.toDateString());
        ...
        // 1. Jika belum ada unique ID atau belum ada tanggal sama sekali
        if (!uniqueId || !uniqueIdDate) {
            generateAndSendUID();
        }
        // 2. Jika sudah ada, cek apakah tanggal kunjungannya berbeda dengan hari ini
        else {
            // toDateString() secara otomatis mencakup Hari, Bulan, dan Tahun.
            // Jika string-nya berbeda, berarti sudah berganti hari.
            if (uniqueIdDate !== currentDate.toDateString()) {
                generateAndSendUID(); // Otomatis menimpa (overwrite) data lama di local storage
            }
        }
    }
